...
import { CYBERSECURITY_PAGE_DATA } from "@/data/cybersecurity-page-data";

export default async function CybersecurityPage({ params: { locale } }) {
  const {
    metadata,
    pageHeader,
    sidebarCategories,
  ...
  return (
    <>
      <SEOHead
        ...
        locale={locale}
        pageUrl="/cybersecurity-services-saudi-arabia"
      />
      ...
      <section className="container mx-auto px-4 sm:px-6 md:px-4 xl:px-12 py-8 sm:py-12 md:py-16 lg:py-20">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
          <div className="lg:col-span-2 space-y-10">
            <ImageSlider
              images={cybersecurityData.images}
              altText={"Cybersecurity Images"}
            />
            <Header
            ...
            >
              <Heading className="!text-primary lg:!text-4xl sm:!text-2xl md:!text-3xl">
                {cybersecurityData.title}
              </Heading>
              <Description>{cybersecurityData.description}</Description>
            </Header>
        ...
        <ProductTypeCard ProductTypes={CYBERSECURITY_TYPES} />
        <IndustryApplications applications={CYBERSECURITY_APPLICATIONS} />
        <WhyChooseUs whychooseus={whychooseus} />
        <FaqAccordion faqs={CYBERSECURITY_FAQS.details} />
      </section>
    </>
  );
}
